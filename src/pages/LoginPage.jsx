	...
	const handleLogin = async (e) => {
		...
		try {
			...
			if (response.data.qrCode) {
				...
				navigate("/verify-email", { state: { qrCode: response.data.qrCode } });
			} else {
    ...
    <div className="w-full max-w-lg p-8">
      ...
      <form onSubmit={handleLogin} className="space-y-4">
        ...
        <div className="flex items-center mb-6">
          <Link
            to="/forgot-password"
            className={`text-sm ${lockoutTime ? "text-gray-400 pointer-events-none" : "text-purple-400 hover:underline"}`}
